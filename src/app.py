...
with st.sidebar.expander("🔑 Connection & API Settings", expanded=False):
    monday_token = st.text_input(
        ...
        type="password",
        help="Personal API Token v2 from Monday.com Developers section."
    )
    ...
    gemini_key = st.text_input(
        ...
        type="password",
        help="Google Gemini API Key for natural language synthesis."
    )
...
def fetch_and_sync_boards(token: str, deals_id: str, wo_id: str):
    """Fetch and normalize records dynamically from Monday.com API."""
    if not token or not deals_id or not wo_id:
        return None, None, {"status": "unconfigured", "message": "Missing API credentials or Board IDs."}

    try:
        client = MondayDataClient(api_token=token)
        health = client.health_check()
        if health.get("status") != "connected":
            return None, None, {"status": "error", "message": health.get("message", "API auth failed.")}

        deals_df = client.get_board_dataframe(deals_id)
        wo_df = client.get_board_dataframe(wo_id)

        from src.normalization import normalize_deals_df, normalize_work_orders_df
        clean_deals = normalize_deals_df(deals_df)
        clean_wo = normalize_work_orders_df(wo_df)

        return clean_deals, clean_wo, {"status": "connected", "user": health.get("user", {})}
    except Exception as e:
        return None, None, {"status": "error", "message": str(e)}


# Trigger reload if refresh requested
if refresh_btn:
    st.cache_data.clear()

deals_df, wo_df, conn_status = fetch_and_sync_boards(monday_token, deals_board_id, wo_board_id)

# Render Connection Health Status
st.sidebar.markdown("---")
...
elif conn_status.get("status") == "unconfigured":
    st.sidebar.warning("⚠️ Credentials not fully configured. Enter credentials in sidebar or .env.")
else:
    st.sidebar.error(f"❌ Connection Error: {conn_status.get('message')}")
...
with st.sidebar.expander("🔍 Board Data Quality Audit", expanded=False):
    if deals_df is not None and wo_df is not None:
